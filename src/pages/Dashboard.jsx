...
function Summary({ items }) {
  ...
  onSite.forEach(i => {
    ;(i.areas || []).forEach(a => {
      if (perArea[a] !== undefined) perArea[a]++
    })
  })
    ...
    <section>
      ...
      <div className="bg-white border rounded mb-6">
        ...
        <Table>
          <thead>
            <tr>
              <Th>Name</Th><Th>Company</Th><Th>Phone</Th><Th>Areas</Th><Th>Fob #</Th><Th>Signed in by</Th><Th>Fob returned</Th><Th>Sign-out requested</Th><Th></Th>{isAdmin && <Th></Th>}
            </tr>
          </thead>
          <tbody>
            {onSite.length === 0 && <tr><Td colSpan={isAdmin ? 10 : 9} className="text-center text-slate-500">None</Td></tr>}
            {onSite.map(i => (
